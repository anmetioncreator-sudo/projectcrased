...
def run_prisma_setup():
    ...
    try:
        subprocess.run([sys.executable, "-m", "prisma", "generate", "--schema", str(BASE_DIR / "schema.prisma")], check=True)
        subprocess.run([sys.executable, "-m", "prisma", "db", "push", "--schema", str(BASE_DIR / "schema.prisma")], check=True)
...
async def start_server():
    ...
    print(f"[Server] Starting Uvicorn server on http://0.0.0.0:{port} ...")
...
async def run_both():
    run_prisma_setup()
    print("[Launcher] Starting both Gateway Server and Discord Bot concurrently...")
